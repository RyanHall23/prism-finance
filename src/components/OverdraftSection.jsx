...
function OverdraftSection({ overdraft, setOverdraft }) {
  const [formData, setFormData] = useState(overdraft);
  ...
  const handleSubmit = (e) => {
    ...
    setOverdraft(updatedOverdraft);
    alert('Overdraft details saved!');
  };
          ...
          <Grid container spacing={2}>
            ...
            <Grid item xs={12} sm={6}>
              <TextField
                ...
                onChange={handleChange}
                inputProps={{ step: '0.01', min: '0' }}
                helperText="Amount currently used from overdraft"
              />
            </Grid>
            <Grid item xs={12}>
              <Button type="submit" variant="contained" startIcon={<Save />}>
                Save Overdraft Details
              </Button>
            </Grid>
          </Grid>
        </form>
      ...
      <Paper sx={{ p: 3 }}>
        ...
        <Grid container spacing={2}>
          <Grid item xs={12} sm={6} md={3}>
            <Typography color="text.secondary">Overdraft Limit</Typography>
        ...
        {parseFloat(formData.currentBalance || 0) > parseFloat(formData.limit || 0) && (
          <Alert severity="error" sx={{ mt: 2 }}>
            Current balance exceeds overdraft limit!
          </Alert>
        )}
      </Paper>
    </Box>
